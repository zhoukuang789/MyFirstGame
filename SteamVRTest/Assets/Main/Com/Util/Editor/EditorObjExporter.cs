...
namespace Tabasco.Common
{
    ...
    public class EditorObjExporter : ScriptableObject
    {
        ...
        private static string MeshToString(MeshFilter mf, Dictionary<string, ObjMaterial> materialList)
        {
            ...
            var meshName = mf.name;

            return MeshToString( materialList, m, mats, transform, meshName );
        }

        private static string MeshToString( Dictionary<string, ObjMaterial> materialList, Mesh m, Material[] mats, Transform transform, string meshName )
        {
        ...
        private static void MeshesToFile(Mesh[] m, Material[] mats, Transform transform, string folder, string filename )
        {
            Dictionary<string, ObjMaterial> materialList = PrepareFileWrite();
            ...
            using ( StreamWriter sw = new StreamWriter( folder + "/" + filename + ".obj" ) ) {
                ...
                for ( int i = 0; i < m.Length; i++ ) {
                    sw.Write( MeshToString( materialList, m[i], mats, transform, m[i].name ) );
                }
            }
        ...
        [MenuItem("Custom/Export/Skinned meshes to file")]
        static void ExportSkinnedMeshToSingle()
        {
            if ( !CreateTargetFolder() )
                return;
            ...
            foreach ( var renderer in selection.SelectMany( s => s.GetComponentsInChildren<SkinnedMeshRenderer>() ) ) {
                var mesh = new Mesh();
            ...
            MeshesToFile( meshes.ToArray(), selection.SelectMany( s => s.GetComponentsInChildren<Renderer>() ).SelectMany( r => r.sharedMaterials ).ToArray(), selection[0].transform, targetFolder, filename );
            EditorUtility.DisplayDialog( "Objects exported", "Exported objects to " + filename, "" );
        }
    }
}
